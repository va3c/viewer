...
//	JAPL.values = {}; // current or runtime updates
	...
	JAPL.refreshParameters = function () {
		...
		'<div>' +
			...
			'<p>' +
				'<small><i><a href="http://en.wikipedia.org/wiki/Permalink" target="_blank">Permalinks</a> ' +
				'enable you to save a scene you have created and send it as a link you can share...</i></small>' +
			'</p>' +
...
// Meier's parametric equations
		...
		var fname = V3MH.files[ item ][ 0 ] + '.html';

// vA3C Objects
		items = ['DrMajentaKlein.json','Hex_01.json','TypTower.json'];
		item = items[ Math.floor( files.length * Math.random() ) ];
...
// target
		...
		for (var i = 0, len = scene.children.length; i < len; i++) {
			...
			if ( obj.geometry ) {
				...
				txt += '#rotx=' + obj.rotation.x;
				txt += '#roty=' + obj.rotation.y;
				txt += '#rotz=' + obj.rotation.z;

				txt += '#sclx=' + obj.scale.x;
				txt += '#scly=' + obj.scale.y;
				txt += '#sclz=' + obj.scale.z;

/*

// creates sparse permalinks
...
// pull in a sample parametric equation
			values = JAPL.addValues();
			items = [ 1, 11, 12, 14, 33, 42, 54, 58, 62, 131, 151, 155 ];
			item = items[ Math.floor( items.length * Math.random() ) ];
			fileTitle = V3MH.files[ item ][ 1 ];
			basepath = V3MH.basepath + '/' + V3MH.files[ item ][ 0 ] + '/';
			fname = V3MH.files[ item ][ 0 ] + '.html';
/*
			hashes =
